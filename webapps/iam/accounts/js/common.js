...
var ZAConstants = {
	...
	getIAMUrl : function(path){
		return this.getAccountsServer() + iam_contextpath + path;
	}
};
...
function reloadCaptcha(idx, f, cdigest) {
	...
	if (!f.captcha.disabled) {
		var captchaCallback = function(cdigest) { // No I18N
			var $captcha = $(f).find(".za-captcha").attr("src", ZAConstants.getAbsoluteURL(ZAConstants.contextpath + "/captcha?cdigest=" + cdigest)); // No I18N
			if (!f.cdigest) {
		...
		if (cdigest) {
			...
			var options = {
				url : ZAConstants.getAbsoluteURL(ZAConstants.contextpath + "/captcha"), // No I18N
				type : "get", // No I18N
				data : {
