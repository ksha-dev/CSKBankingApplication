...
(function($) {
	...
	$.fn.zaSignUp.defaults = {
		...
		url : function() {
			if($.fn.zaSignUp.defaults.x_signup.otp_required){
					return ZAConstants.getAbsoluteURL(ZAConstants.contextpath + "/register/otp/initiate"); // No I18N
			}else{
				return ZAConstants.getAbsoluteURL(ZAConstants.contextpath + "/register.ac"); // No I18N
			}
	...
	$.validator.addMethod("validateRemote", function(value, element, param) { // No I18N
		...
		if (element.name.indexOf(CrossServiceRequest.CONSTANTS.CUSTOM_FIELD) == 0) {
			...
			options = {
				url : ZAConstants.getAbsoluteURL(ZAConstants.contextpath + "/validate/register.ac"), // No I18N
				type : "POST", // No I18N
				data : pdata
			};
		...
		if (data.httpResponseCode == "400" && representation.redirect_uri) {
			...
			if (errorCode == "A110") {
				...
				if(data.errorCause.indexOf("Invalid Mobile Number") > 0){
					errorObj.emailormobile = I18N.get("IAM.PHONE.ENTER.VALID.MOBILE_NUMBER");
					errorObj.mobile = I18N.get("IAM.PHONE.ENTER.VALID.MOBILE_NUMBER");
				}else{
				...
				errorObj.email = I18N.get("IAM.ERROR.CODE.U131");
			} else {
...
function customFieldAudit(field, error) {
	$.post(ZAConstants.getAbsoluteURL(ZAConstants.contextpath + "/regaudit/customfield"), { // No I18N
		field : field,
		error : error
	});
}
...
function validateOTP(){
	...
	var validateoptions = {
			url : ZAConstants.getAbsoluteURL(ZAConstants.contextpath + "/register/otp/validate"), // No I18N
			type : "POST", // No I18N
			data : params,
...
function resendOTP(){
	...
	var validateoptions = {
			url : ZAConstants.getAbsoluteURL(ZAConstants.contextpath + "/register/otp/resend"), // No I18N
			type : "POST", // No I18N
			data : params,
			success:function(data){
		    	  	if(data.error){
...
function otpInitiateResponse(ar){
	if(ar.json){
		if(ar.json.status=="success"){
			...
			$(".loadingImg").hide();
			$('form[name="signupform"]').attr("action",ZAConstants.getAbsoluteURL(ZAConstants.contextpath+"/register/otp/validate")); // no i18n
		}
	}
}
