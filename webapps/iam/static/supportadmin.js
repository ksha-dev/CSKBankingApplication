...
function getAppServices(f){
	...
	var resp = getPlainResponse("/support/defservice/fetch", params); //No I18N
	if(isJson(resp)){
		var obj = JSON.parse(resp);
		if((obj.APPSERVICE==null ||obj.APPSERVICE.length <1) && obj.DEFAULT == null){
			showerrormsg("No Service For the User" + obj.APPSERVICE.length); //No I18N
			return false;
		}
		var list = "<br><br><table class ='usremailtbl' border='1' cellpadding='4'><tr><th style='padding:0px' class='usrinfoheader'>ZID</th><th style='padding:0px' class='usrinfoheader'>ORG NAME</th><th style='padding:0px' class='usrinfoheader'>Action</th></tr>"; //No I18N
		if(obj.DEFAULT!=null) {
			list+= "<tr><td width='40%' align='center'>"+obj.DEFAULT.ZID+"</td><td width='40%' align='center'>"+obj.DEFAULT.ORG_NAME+"</td><td width='40%'  style='color: #005bec;font-weight: bold;' align='center'>Default</td></tr>"; //No I18N
		}
		for(var i=0;i<obj.APPSERVICE.length;i++){
			var appSer = obj.APPSERVICE[i]; align='center'
			var button = "<div onclick='makeThisDefault("+appSer.ZID+","+appSer.ORG_TYPE+","+appSer.ZUID+")'><span class='bluesavebtn cbtnco'>Make This Default</span></div>"; //No I18N
			list += "<tr><td width='44%' align='center'>"+appSer.ZID+"</td><td width='44%' align='center'>"+appSer.ORG_NAME+"</td>"; //No I18N
			if(obj.APPSERVICE.length>0 && (obj.DEFAULT == null || obj.DEFAULT.ZID != appSer.ZID)){
		...
		list+="</table>"; //No I18N
		de("appservicedetails").innerHTML = list; //No I18N
		de("appservicedetails").style.display='block'
	}
	else{
		showerrormsg(resp);
	}
...
function makeThisDefault(zid,type,zuid){
	var params = "zid="+euc(zid)+"&type="+euc(type)+"&zuid="+euc(zuid)+"&"+csrfParam; //No I18N
	if(confirm("Are You Sure?")){ //No I18N
		showverificationform("/support/defservice/set", params, setDefaultResponse); //No I18N
	}
	return false;
}
...
function setDefaultResponse(resp){
	if(resp=="SUCCESS"){ //No I18N
		hideverificationform();
		getAppServices(document.useremail);
...
function getServiceDetails(f){
	var zid = f.zid.value.trim();
	if(isEmpty(zid)){
		showerrormsg("Invalid ZID passed");  //No I18N
		return false;
	}
	var type = f.servicetype.value.trim();
	if(type==''){
		showerrormsg("Select A Valid Service"); //No I18N
		return false;
	}
	var params = "zid="+euc(zid)+"&type="+euc(type)+"&"+csrfParam; //No I18N
	var resp = getPlainResponse("/support/servicedetails/fetch", params); //No I18N
	if(isJson(resp)){
		var obj = JSON.parse(resp);
		if(obj.APPSERVICE==null) {
			showerrormsg("No Service For the User"); //No I18N
			return false;
		}
		var list = "<br><br><table class ='service_details' border='1' cellpadding='4'><tr><th style='padding:0px' class='usrinfoheader'>ZID</th><th style='padding:0px' class='usrinfoheader'>ORG NAME</th><th style='padding:0px' class='usrinfoheader'>STATUS</th><th style='padding:0px' class='usrinfoheader'>Action</th></tr>"; //No I18N
		var appSer = obj.APPSERVICE; align='center' //No I18N
		var status = appSer.ACCOUNT_STATUS == 1 ? "Active" : appSer.ACCOUNT_STATUS == 0 ? "InActive" : "Closed"; //No I18N
		var button = "<div onclick='closeAppService("+appSer.ZID+","+appSer.ORG_TYPE+")' style=' margin: auto; width: max-content;'><span class='blueclosebtn cbtnco'> Close </span></div>"; //No I18N
	    list += "<tr><td width='20%' align='center'>"+appSer.ZID+"</td><td width='40%' align='center'>"+appSer.ORG_NAME+"</td><td width='20%' align='center'>"+status+"</td>"; //No I18N
	    if(appSer.ACCOUNT_STATUS!=2){
				list+= "<td>"+button+"</td></tr>"; //No I18N
		} else{
				list+= "<td align='center'><div style='color : #7e7e7f;'>No Options</div></td></tr>"; //No I18N
		}

		list+="</table>"; //No I18N
		de("servicedetails").innerHTML = list; //No I18N
		de("servicedetails").style.display='block'
	}
	else{
		showerrormsg(resp);
	}
...
function closeAppService(zid,type) {
	var params = "zid="+euc(zid)+"&type="+euc(type)+"&"+csrfParam; //No I18N
	if(confirm("Are You Sure?")){ //No I18N
		showverificationform("/support/servicedetails/close", params, closeResponse); //No I18N
	}
	return false;
}

function closeResponse(resp){
	if(resp=="SUCCESS"){ //No I18N
		hideverificationform();
		getServiceDetails(document.zid);
		showsuccessmsg("SUCCESSFULLY CLOSED"); //No I18N
	}
	else{
		if(resp!="INVALID_ADMINPASSWORD"){ //No I18N
			hideverificationform();
...
function updateOwner(f) {
	var params = "";
	try{
		var zid = f.zid.value.trim();
	    if(isEmpty(zid) || isNaN(zid)){
		   showerrormsg("INVALID ZID"); //No I18N
		   throw "";
	    }
	    var subtype = f.subtype.value.trim();
	    if(isEmpty(subtype) || subtype==''){
		...
		params = csrfParam +"&zid="+euc(zid)+"&subtype="+euc(subtype)+"&"; //No I18N
	}catch (e) {
		return;
	}
	...
	params += "zuid="+euc(zuid); //No I18N
	if(confirm("Want to change Owner?")){ //No I18N
		showverificationform("/support/owner/update", params, ownerResponse) //No I18N
	}
	return false;
}

function ownerResponse(resp){
	if(resp == "SUCCESS"){ //No I18N
		hideverificationform();
		showsuccessmsg(resp);
	}
