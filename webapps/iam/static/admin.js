...
function saveService(f) {
	var isAddService = (f.name == "addservice") ? true : false;
    if(isEmpty(f.serviceName.value) || isEmpty(f.displayName.value) || isEmpty(f.description.value) || isEmpty(f.domainName.value) || isEmpty(f.homePage.value) || isEmpty(f.orgType.value) || (isAddService && isEmpty(f.orgModel.value))) {
    ...
    param = param + "&orgType="+f.orgType.value;//No I18N
    ...
    if(isAddService) {
		...
		if(f.orgModel.value = "true"){
			param = param + "&orgModel="+f.orgModel.value;//No I18N
		}
    }
    else {
        url = "/admin/updateservice"; // No I18N
        param = param + "&serviceid=" + euc(f.serviceid.value); // No I18N
		if(isServicOrgEnabled){
			param = param + "&serviceorgaccounttype="+f.serviceorgaccounttype.value;//No I18N
		}
		if(isAppAccountEnabled){
			param = param + "&appaccaccounttype="+f.appaccaccounttype.value;//No I18N
		}
    }
...
function bitsetParser() {
    var val = de('val').value.trim(); // No I18N
    if(isEmpty(val)) {
        showerrormsg("Enter the value to search"); // No I18N
        return false;
    } else {
		var type = de('type').value.trim();// No I18N
    	loadui("/ui/admin/bitsetparser.jsp?"+"val="+ euc(val) +"&type="+ euc(type)); // No I18N
    	return false;
    }
}
...
function restoreAccount(f) {
    ...
    if(isEmpty(userid) || isNaN(userid)) {
        ...
        f.email.focus();
    }else if(isEmpty(comment) || comment == "null") {  //No I18N
        showerrormsg("Please enter valid comment"); // No I18N
        f.comment.focus();
    }else if(isEmpty(pwd)) {
        showerrormsg("Please enter administrator password"); // No I18N
        f.pwd.focus();
    } else {
        var params = "userid=" + euc(userid) + "&email=" + euc(email) + "&pwd=" + euc(pwd) + "&comment=" + euc(comment) + "&" + csrfParam; //No I18N
...
function callInternalinvokeIps(iplist){
	...
	var usecase = "";
	var useCaseVar = $(".checkusecaselist #usecaseclearcache");
	var len = $(".checkusecaselist").length 
	for(var i=0;i <len;i++) {
		if(useCaseVar[i].checked){
			usecase += useCaseVar[i].value+",";
		}
	}
	if(usecase.length > 0){
		usecase = usecase.slice(0, -1);
	}
	if(usecase != null){
		param = param + "&usecases=" + usecase;	// No I18N
	}
	var resp = getPlainResponse(contextpath + "/internal/invoke.jsp", param); // No I18N
	if(resp.indexOf("{}")!=-1){
		var message=admin?"cache cleared in selected app servers":"cache cleared in all app servers";//No I18N
...
function validateinputstogenerateclientid(locations_count,currentloc){
	...
	var jsDomainRegex = clientform.jsDomainRegex.value;
	var redirectUriRegex = clientform.redirectUriRegex.value;
	...
	if(appgroupID == "1000_24" || appgroupID == "1000_25"){
		if(!jsDomainRegex || jsDomainRegex.trim() == ""){
			alert('Enter valid JS Domain Regex'); // No I18N
			document.getElementById('jsDomainRegex').innerHTML = "";
			return; 
		}
		if(!redirectUriRegex || redirectUriRegex.trim() == ""){
			alert('Enter valid RedirectUri Regex'); // No I18N
			document.getElementById('redirectUriRegex').innerHTML = "";
			return;
		}
	}
	if(!password || password.trim() == "" ) {
	...
	if(jsDomainRegex || !jsDomainRegex.trim() == ""){
		params += "jsDomainRegex=" + euc(jsDomainRegex) + "&"; // No I18N
	}
	if(redirectUriRegex || !redirectUriRegex.trim() == ""){
		params += "redirectUriRegex=" + euc(redirectUriRegex) + "&"; // No I18N
	}
...
function validateRESTInputs() {
	...
	var comment = restfrm.comment.value;
	if(!comment || comment.trim() == "" ){ 
    	showerrormsg("Please enter the comment");  //No I18N
    	return;
	}
	params += "&comment=" + euc(comment); //No I18N
	...
	if(methodval == "get"){
		url = '/admin/rest/get?';//No I18N
		isValidReq = true;
	}
	else if(confirm("Do you confirm to do '" + methodval.toUpperCase() + "' operation?")){
		url = '/admin/rest/update?'	  //No I18N
		isValidReq = true;
	}
	if(isValidReq) {
		$.ajax({
			...
			success: function(data) {
				...
				if(json.httpResponseCode == 200 || json.httpResponseCode == 404 ||json.response == "success") {
					...
					if(json.message == "invalid_data") {
						...
						showerrormsg("Invalid password"); // No I18N
						return;
					}else if(json.message == "invalid_comment"){  //No I18N
						showerrormsg("Invalid comment"); // No I18N
						return;
					}
				}
				showerrormsg("Error code :: " + json.httpResponseCode); // No I18N
			},
			error: function(data) {
				restfrm.password.value = '';
				var json = JSON.parse(JSON.stringify(data));
				if(methodval == "get" && json.responseJSON != undefined && json.responseJSON.cause === "invalid_password_token"){
						openReauthWindow(json.responseJSON);
						return;
					}
				else{
...
function switchTFAClass(element) {
	if(element.id == "enabletfaheader") {
		...
		de("genrecoveryredirect").style.display="none";
		de("genrecoverylink").style.display="none";
...
function generateRecoveryCode(redirecturl)
{
	$("#enabletfaheader").removeClass("tfasel");
	$("#resettfaheader").removeClass("tfasel");
	$("#smsstatusheader").removeClass("tfasel");
	$("#generateRecoveryCodeheader").addClass("tfasel");
	de('enabletfa').style.display="none";
	de('resettfa').style.display="none";
	de('smsstatus').style.display="none";
	de("genrecoveryredirect").style.display="";
	de("genrecoverylink").style.display="";
	de("genrecoveryredirect").innerHTML = "<p>Please generate Recovery codes from AOC: </p>";//No I18N
	de('genrecoverylink').innerHTML = `<a href='${redirecturl}' target='_blank'>Click here</a>`;
	}
...
function deleteCluster(cluster) {
	if(confirm("Are you sure to delete "+cluster+" ?")) {//No I18N
        var params ="op=delete&cluster="+euc(cluster)+"&"+csrfParam; //No I18N
  		sendRequestWithCallback(contextpath+"/admin/cluster/op", params, true, clusterResponse); //No I18N
    }
    return false;
}
...
function delNode(cluster,node) {
	if(confirm("Are you sure to delete "+node+" from "+cluster+" cluster?")) {//No I18N
    	...
    	sendRequestWithCallback(contextpath+"/admin/cluster/node/op", params, true, clusterResponse); //No I18N
    }
    return false;
}
...
function clusterResponse(resp){
	 resp = resp ? resp.trim() : "";//No I18N
	    if(resp === "SUCCESS") {//No I18N
	        ...
	        loaduiviaPOST('/ui/admin/cluster/clusteradmin.jsp?op=view', "", 'clusteradmin');//No I18N
	    }
	    else {
...
function fetchOrgdetails(f) {
    ...
    if(obj.paid_services != null) {
    	de("idspaidservice").value = obj.paid_services;// No I18N
    	document.getElementById("merge").style.pointerEvents = 'none';// No I18N
    }
    ...
    if(obj.dataloss_appaccounts!=null){
	    for (var i = 0; i < obj.dataloss_appaccounts.length; i++) {
			...
			if(obj.merge_appaccounts != null) {
			     var option = document.createElement('option');
			     var destAppacc = obj.merge_appaccounts[i];
			     option.value = appacc.zaaid + "-" + destAppacc.zaaid + "|" + destAppacc.app_name + " - " + destAppacc.zaaid + " - " + destAppacc.screen_name;
			     option.innerText = appacc.app_name + " - " + appacc.zaaid + " - " + appacc.screen_name;
			     de("idslossappaccs").append(option);// No I18N
			    }
		}
		$(".idslossappaccs").show();
    }
    ...
    $("#idsappaccs").select2();
    if(mergedataloss != "") {
		de("idslossappaccs").append(mergedataloss);// No I18N
    	$(".idslossappaccs").show();
	}
...
function mergeOrgdetails(f) {
	...
	var mergeAppAccountResult = "";
	var child_div = $(".edipdiv");
	for (var i = 0; i < child_div.length; i++) {
		var selectDiv = child_div[i].getElementsByTagName('select')[0];//No I18N
		var selectedValue = selectDiv.value;
		if(selectedValue == '') {
			continue;
		}
		var zaaid = selectedValue.split("|")[0];
		if(mergeAppAccountResult.length != 0) {
			mergeAppAccountResult += ",";
		}
		mergeAppAccountResult += zaaid;
	}

    var params = "fromOrgID=" + euc(fromOrgID) + "&toOrgID=" + euc(toOrgID) +"&subIds=" + euc(result) +"&mergeIds=" + euc(mergeAppAccountResult)  +"&pwd=" + euc(adminPassword) + "&retailRole=" + f.retailRole.checked  + "&" + csrfParam; //No I18N
    ...
    if(obj.success == "true") {
         ...
         de("idsmergedmig").value =obj.AppAccount_merged;// No I18N	
    } else if(obj.error) {
...
/** Mobile AppDetails Admin Console ends */
	...
	function showId(ele){
		var selectedOption=ele.options[ele.selectedIndex].innerHTML;
		if(selectedOption==='DEVICE-BASED'){
		...
		else if(selectedOption==='EXTERNAL_CLIENTOAUTH_BROWSER_BASED' || selectedOption==='EXTERNAL_CLIENTOAUTH_INSTALLABLE'){
	...
	function countTestAccounts(){
		de('downloadOut').style.display = '';
		de('downloadOut').innerHTML = "Loading Please wait....."; //No I18N
		var params = csrfParam;
		var resp = getPlainResponse(contextpath + "/admin/testaccount/count", params); //No I18N
		var parsedResp = JSON.parse(resp);
		if(parsedResp.hasOwnProperty("error")){
			showerrormsg(parsedResp.error);
			de('downloadOut').style.display = 'none';
			return;
		}
		if(parsedResp.hasOwnProperty("status")){
			downloadTestAccountResponse(parsedResp.status);
		}
	}
	
	function downloadTestAccounts() {
		var params = csrfParam;
		var resp = getPlainResponse(contextpath + "/admin/testaccount/download", params); //No I18N
		var parsedResp = JSON.parse(resp);
		if(parsedResp.hasOwnProperty("error")){
			showerrormsg(parsedResp.error);
			return;
		...
		if(parsedResp.hasOwnProperty("status")){
			...
			downloadTestAccountResponse(parsedResp.localized_message);
		}
	}
	...
	function deactiveTestAccounts(f) {
		if(checkFile(f)){
			const formElement = document.getElementById('deactivateTestAccount');
	  		const formData = new FormData(formElement);
			var resp = sendFileData(contextpath+"/admin/testaccount/deactivate", formData); //No I18N
			var parsedResp = JSON.parse(resp);
			if(parsedResp.hasOwnProperty("error")){
				showerrormsg(parsedResp.error);
				return;
			...
			if(parsedResp.hasOwnProperty("status")){
				deactivateTestAccountResponse(parsedResp.status);
			} else {
				deactivateTestAccountResponse(parsedResp.localized_message);
			}
		}
	}	
	...
	function closeTestAccounts(f) {
		if(checkFile(f)){
			const formElement = document.getElementById('deleteAccounts');
	  		const formData = new FormData(formElement);
			var resp = sendFileData(contextpath+"/admin/testaccount/close", formData); //No I18N
			var parsedResp = JSON.parse(resp);
			if(parsedResp.hasOwnProperty("error")){
				showerrormsg(parsedResp.error);
				return;
			...
			if(parsedResp.hasOwnProperty("status")){
				closeTestAccountResponse(parsedResp.status);
			} else {
				closeTestAccountResponse(parsedResp.localized_message);
			}
		}
	}
	...
	function checkTestAccounts(f) {
		if(checkFile(f)){
			const formElement = document.getElementById('testAccountCheck');
	  		const formData = new FormData(formElement);
			var resp = sendFileData(contextpath+"/admin/testaccount/check", formData); //No I18N
			var parsedResp = JSON.parse(resp);
			if(parsedResp.hasOwnProperty("error")){
				showerrormsg(parsedResp.error);
				return;
			...
			if(parsedResp.hasOwnProperty("status")){
				checkTestAccountResponse(parsedResp.status);
			} else {
				checkTestAccountResponse(parsedResp.localized_message);
			}
		}
	}
	
	function sendFileData(action, params) {
	    var objHTTP = xhr();
	...
	function configureNewMobileApp(f){
		...
		var audienceId = f.audienceId.value.trim();
		var isClientPortal = f.isClientPortal.checked;
		var clientId = f.clientId.value.trim();
		...
		if(isClientPortal && (isEmpty(clientId) || !clientId.startsWith("client."))){//No I18N
			showerrormsg("Enter a valid Client Id");//No I18N
			return false;
		}
		var params = "&serviceName="+euc(serviceName)+"&appName="+euc(appName)+"&packageName="+euc(packageName)+"&clientType="+euc(clientType)+"&mobileappType="+euc(mobileappType)+"&signature="+euc(signature)+ //No I18N
					 "&urlScheme="+euc(urlScheme)+"&appClientId="+euc(appClientId)+"&projectId="+euc(projectId)+"&audienceId="+euc(audienceId)+"&isClientPortal="+euc(isClientPortal)+"&clientId="+euc(clientId)+"&"+csrfParam;//No I18N
	...
	function addClientId(e){
		if(e.checked){
			de('client_portal').style.display = '';
		}
		else{
			de('client_portal').style.display = 'none';
		}
	}
...
function getClientScopes() {
	const client_id = de("client_id").value.trim(); // No I18N
	if(isEmpty(client_id)) {
		showerrormsg("Please enter a valid Client ID"); // No I18N
		de("client_id").focus(); // No I18N
		return;
	}
	var params = "client_id="+euc(client_id)+"&"+csrfParam; // No I18N
	var resp = getPlainResponse(contextpath+"/admin/clientscopes", params); //No I18N
	var textarea = de("clientscopes_resp"); //No I18N
	textarea.value = resp;
