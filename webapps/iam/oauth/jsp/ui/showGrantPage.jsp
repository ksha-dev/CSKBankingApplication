		...
		<div class="screen"></div>
            ...
            <div class="wrap">
	  		...
	  		<% if(!isClientPortal) { %>		
	  			...
	  			<% } else { %>
	  				<img class="logo arattai_logo" src="<%=StaticContentLoader.getStaticFilePath("/v2/components/images/newZoho_logo.svg")%>"><div class="arattai_title"><%=I18NUtil.getMessage("ZOHO.ZohoArattai.DISP")%></div>
				<% } %>	
      		<% } %>  	       
      <% 
      				if(isDirectAccess) 
...
%>
								                    <div class="error" id="<%=subpromptcount%>_error">
								                        ...
								                        <div class="but_action">
								                         <button class="btn" id="next_<%=subpromptcount%>" onclick='change_div(<%=subpromptcount%>,"<%=IAMEncoder.encodeHTML(serviceName)%>", <%=isapplicationBased%>);'><%=I18NUtil.getMessage("IAM.NEXT")%></button>								                        
        ...
        function display_selected()
        {
            ...
            <%
  			...
  			{ %>
  			 ...
  			 <% } else { %>
  				$( "#selectedbox_div" ).append( '<div id="scopes_'+i+'"><ul></ul></div>' );  //No I18N
...
function submitDeviceApproveForm() {
	$(".btn").attr("disabled", "disabled"); //No I18N
	$(".cancel").attr("disabled", "disabled"); //No I18N
	var approvedOrgs = "";
 	if(Object.keys(fullList).length > 0){
	...
	if(jsonStr != null && jsonStr.redirect_uri) {
	 ...
	 <% 	} %>
		...
		$(".btn").removeAttr("disabled"); //NO I18N
		$(".cancel").removeAttr("disabled"); //NO I18N
	}
}
