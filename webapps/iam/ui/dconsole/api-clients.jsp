...
<div class="add_client_main_div">
	...
	%>
		  ...
		  %>
			...
			<div style="padding:10px 0px;">
				<div class="multi_dc_checkbox_div"><%=IAMEncoder.encodeHTML(loc)%></div>
				<div class="multiDc_enable" style="background-color:#fff;color:#000;border:1px solid #c1c1c1;" onclick="multiDcCheckbox('<%=IAMEncoder.encodeHTMLAttribute(loc)%>','<%=IAMEncoder.encodeHTMLAttribute(oauthClient.getClientId())%>','<%=IAMEncoder.encodeHTMLAttribute(oauthClient.getParent().getOauthAppGroupId())%>',this,'Disabled')"><%=Util.getI18NMsg(request, "IAM.MFA.MODE.DISABLED")%></div>
