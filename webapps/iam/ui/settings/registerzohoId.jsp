...
<html>
	...
	<div id="successmsgpanel" class="hide"><span id="successresp">&nbsp;</span><div style="margin-top:10px;display:none;" id="success_mobile_txt"><%=Util.getI18NMsg(request, "IAM.FORGOT.SMS.ALSO.SENT")%></div><a href="<%=IAMEncoder.encodeHTMLAttribute(serviceUrl)%>"><%=Util.getI18NMsg(request, "IAM.OK")%></a></div><%-- NO OUTPUTENCODING --%>
	</div>
	</td>
	</tr>
...
<script>
    ...
    function showHip(cdigest) {
        ...
        hipRow.innerHTML= "<img name='hipImg' id='hip' class='hip' width='200' height='75' src='<%=request.getContextPath()%>/accounts/captcha?cdigest=" + cdigest + "'  alt='<%=Util.getI18NMsg(request,"IAM.HIP.IMAGE")%>' title='<%=Util.getI18NMsg(request,"IAM.IMAGE.VERIFICATION")%>' align='absmiddle'>"; <%-- NO OUTPUTENCODING --%>
    }
