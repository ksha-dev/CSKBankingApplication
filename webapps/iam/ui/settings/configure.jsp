...
<head>
	...
	String uri = (String) request.getAttribute("javax.servlet.forward.request_uri"); //No I18N
	...
	if(uri.equals(cPath + "/ui/configure/mobile/login")){//No I18N
		...
		UserMobile[] temp = PhoneUtil.getUserRecoveryNumbers(user);
...
<div id="error_space">
	...
	<div class="form_field" id="get_mob_no" >
	   ...
	   <select class="chosen-select" id="country_code_select">
		...
		<%}else{%>
		<input class="text_box" id="mobileno" type="email"  type="text" style="text-indent:5px;" required="" autocomplete="off" placeholder="<%=Util.getI18NMsg(request,"IAM.USER.ENTER.EMAIL.PLACEHOLDER")%>" value="<%=email%>"/>
        <span class="edit_icon" onclick=edit();></span>
		<%}%>
    ...
    <button class="btn" id="ver_id" onclick="verifyCode()"><%=Util.getI18NMsg(request,"IAM.NEW.SIGNIN.VERIFY")%></button>    
...
<script>
    ...
    function updateMobile() {
        ...
        if(!doValidate(usedMobile)){
            showErrMsg('<%=Util.getI18NMsg(request, isEmail?"IAM.ERROR.EMAIL.INVALID":"IAM.PHONE.ENTER.VALID.MOBILE_NUMBER")%>')
            return;
        }
        <%if(isEmail){%>
