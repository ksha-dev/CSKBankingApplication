...
<div id="error_space">
	...
	<div class="form_field" id="get_mob_no" >
     ...
     <div class="form_field" id="get_password">
        ...
        <span class="bar"></span>
        <label><%=Util.getI18NMsg(request,"IAM.NEW.SIGNIN.PASSWORD")%></label>
    </div>
		...
		if(temp && temp.type==undefined && temp!='hide'){
			...
			if($('#mobile_no_select').val()=='new'){
				...
				if($('#get_mob_no')){
					$('#get_mob_no').hide();
					$('#mob-label').show();
					de('send_ver').innerHTML="<%=Util.getI18NMsg(request,"IAM.NEW.SIGNIN.VERIFY")%>"; //No I18N	
				}
			}
		}
	}
    ...
    function updateMobile(){
    	...
    	if($('#mobile_no_select') && $('#mobile_no_select').val()!='new'){//No I18N
            ...
            if(!isValidMobile(usedMobile)){
                showErrMsg('<%=Util.getI18NMsg(request, "IAM.PHONE.ENTER.VALID.MOBILE_NUMBER")%>')
                return;
            }
