...
<html>
	...
	<div style="margin-top:12px;">
		...
		<div class="descRtd txtalignlft">
			<span class="bluebutton" onclick="verifyotp(document.password);"><%=Util.getI18NMsg(request, "IAM.NEW.SIGNIN.VERIFY")%></span> <%-- NO OUTPUTENCODING --%>
			<%if(isForgotPassword){ %>
...
<table width="100%" height="100%" align="center" cellpadding="0" cellspacing="0">
	...
	<div class="label">
		...
		<div class="button">
			<span <%if(isMobile){%>class="bluebutton" <%}else {%>class="redBtn"  style="<%=isForgotPassword? "" : "margin-right:150px;" %> " id="buttonloader verifyCode" <%} %>  tabindex="1"  onclick="verifyotp(document.password);"><%=Util.getI18NMsg(request, "IAM.NEW.SIGNIN.VERIFY")%><span class="loadingImg" Style="display:none"></span></span> <%-- NO OUTPUTENCODING --%>
			<%if(isForgotPassword) {%>
	...
	var isIOSDevice = '<%=isIosDevice%>';
	 ...
	 function updateMobile(){
	 		...
	 		if(!isValidMobile(mobile)){
	 			showError('<%=Util.getI18NMsg(request, "IAM.PHONE.ENTER.VALID.MOBILE_NUMBER")%>')
	 			return;
	 		}
	 ...
	 function editMobile(){
	     	...
	     	$('#cancel').html('<%=Util.getI18NMsg(request, "IAM.AC.CONTACT.SUPPORT.GO_BACK")%>');//No I18N
