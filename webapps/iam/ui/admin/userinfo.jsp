...
%>
		...
		if(app.getAccountStatus() == AccountsConstants.AppAccountStatus.ACTIVE) {
			...
			for(int status : statusList) {
				try{
					...
					LOGGER.log(Level.WARNING,"Exception occured while getting appAcc Internal Member Count -- OrgType {0}, ZAAID {1}", new Object[]{app.getOrgType(), app.getZAAID()});
                }
			}
			if(ort != null) {
					...
					if(hasExternal) {
						...
						for(int status : statusList) {
							try {
								...
								LOGGER.log(Level.WARNING,"Exception occured while getting appAcc External Member Count -- OrgType {0}, ZAAID {1}", new Object[]{app.getOrgType(), app.getZAAID()});
                            }
						}
					}
				}
		...
		<% if(internalCount.length > 0) {
				if(internalCount[0] != null) {
                    ...
                    <%} else {%>
                        <div><span style="color: #999;">No Internal Members</span><br></div> <%-- NO OUTPUTENCODING --%> <%-- No I18N --%>
					<%}
				}
		} %>
		<% if(hasExternal && externalCount.length > 0) {
				if(externalCount[0] != null) {
					if(externalCount[0] > 0) {%>
						<div><span style="color: #999;">External Members:</span><br> <span style="color: #999;">Total Members</span>: <%= externalCount[0] %>, <span style="color: #999;">Active Members</span> : <%= externalCount[1] %>, <span style="color: #999;">InActive Members</span> : <%=externalCount[2]%></div>  <%-- NO OUTPUTENCODING --%> <%-- No I18N --%>
					<%} else {%>
						<div><span style="color: #999;">No External Members</span><br></div> <%-- NO OUTPUTENCODING --%> <%-- No I18N --%>
					<% }
				}
			}
		}%>
