<%-- $Id$ --%>
<%@page import="com.zoho.accounts.internal.util.CryptoUtil"%>
...
<% if (Util.isValid(value)) {
		if(type != null) {
			...
			if(digestObj == null && type.equals("DIGEST") && !AccountsConfiguration.getConfigurationTyped("enable.digest.ear.encryption", false)){
				criteria = new Criteria(DIGEST.DIGEST,com.adventnet.iam.CryptoUtil.encrypt(com.adventnet.iam.internal.Util.IAM_SECRET_KEYLABEL, value));
				digestURI = (Accounts.DigestURI)digestURI.getQueryString().setCriteria(criteria).setOrderBy(DIGEST.CREATED_TIME.toString(), false).setLimit(1, 25).build();
			    digestObj = digestURI.GETS(); 
						...
						<td>
						    <table cellpadding="3">
								<tr>
								    ...
								    <td class="usrinfotdrt"><%=CryptoUtil.EAREncrypt(com.adventnet.iam.internal.Util.IAM_SECRET_KEYLABEL, value,true)%></td><%-- NO OUTPUTENCODING --%>
								</tr>
							</table>
						</td>
					</tr>
			...
			<% } else {
				...
				try {
					decryptedValue = CryptoUtil.EARDecryptWithBackwardCompatibility(com.adventnet.iam.internal.Util.IAM_SECRET_KEYLABEL, value,true);
			    } catch(Exception e) {
