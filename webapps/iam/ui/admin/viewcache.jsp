...
<%@page import="com.zoho.jedis.v390.Tuple"%>
...
private String[] convertToString(Object memCacheValue) {
	...
	try {
		...
		if(val instanceof Message){
			return new String[]{dat, String.valueOf(val)};
		}
		return new String[]{dat};
	} catch(Exception e) {
		LOGGER.log(Level.FINE, null, e);
	}
	return new String[]{String.valueOf(memCacheValue)};
}
	...
	if (isKey) {
		...
		case "string" : //No I18N
			if(poolType.equalsIgnoreCase(Pools.EPHEMERAL_POOL.getPoolName())) {
				...
				map.put(poolType + splitKey + uniqueId, node.get(uniqueId) == null ? null : convertToString(node.get(uniqueId))[0]);
			} else {
				Object value = MemCacheUtil.getValue(MemCacheConstants.valueOf(poolType), new MemCacheKey(uniqueId));
				map.put(poolType + splitKey + uniqueId, value == null ? null : convertToString(value)[0]);				
			}
			break;
		case "set" : //No I18N
				...
				switch (datatype){
					case "string" : //No I18N
					    if (MemCacheUtil.isInternalNullValue(memValue)) {
					    	...
					    	if(memValue == null) {
							    out.print(IAMEncoder.encodeHTML(null));
					    	} else {
						    	String[] values = convertToString(memValue);
						    	if(values.length == 1) {
							    	value = values[0];
								    out.print(IAMEncoder.encodeHTML(value));
						    	} else {
								    out.print(IAMEncoder.encodeHTML(values[0]) + "<br><br>" + IAMEncoder.encodeHTML(values[1]));
						    	}
					    	}
					    }
					   	break;
