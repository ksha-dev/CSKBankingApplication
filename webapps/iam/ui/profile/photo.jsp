					...
					<%if (type == null || "user".equals(type)) {//No i18n %>
						<div class="label">
							<label class="inlineLabel"><%=Util.getI18NMsg(request, "IAM.PHOTO.VIEW.PERMISSION")%>  </label> <label> <select name="photoViewPermission" id="photoViewPermission_pop" class="photoselect inputSelect chosen-select"> 
									<option value="<%=UserPreference.PHOTO_VIEW_ZOHO%>"><%=Util.getI18NMsg(request, "IAM.PHOTO.PERMISSION.ZOHO_USERS")%></option>  <%-- NO OUTPUTENCODING --%>
									...
									<option value="<%=UserPreference.PHOTO_VIEW_NONE%>"><%=Util.getI18NMsg(request, "IAM.PHOTO.PERMISSION.ONLY_MYSELF")%></option>  <%-- NO OUTPUTENCODING --%>
							</select>
