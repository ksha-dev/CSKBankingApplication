...
if(AccountsConfiguration.getConfigurationTyped("reauth.v1.force.redirect", true)) {
	response.sendRedirect(Util.getServerUrl(request) + AccountsConfiguration.getConfiguration("reauth.url.path", "/account/v1/relogin") + (request.getQueryString() == null ? "" :"?"+request.getQueryString()));//No I18N
	return;
}
...
function checkpassword(f){
	...
	if(isEmpty(passwd)){
		$('#relogin #pwd').focus();
 		checkPwdVal('<%=Util.getI18NMsg(request, "IAM.ERROR.ENTER.LOGINPASS")%>');
    	return false;
	}
...
<form class="zform"  name="pcform"id="relogin" novalidate="novalidate" method="post">  
        ...
        <div class="zoho_logo"></div>
	        ...
	        <div class="mail_id">
	        	...
	        	<span class="notyoulink"  onclick="return logoutFuntion();"><%=Util.getI18NMsg(request, "IAM.NOT.YOU")%></span></span> 
		...
		%>
			...
			{
			%>
			<span class="bluetext_action" onclick="return samlUser()"><%=Util.getI18NMsg(request, "IAM.NEW.SIGNIN.USING.SAML")%></span>
			<%
			}
			%>
		...
		{
		%>
		<button class="btn" onclick="return samlUser()"><%=Util.getI18NMsg(request, "IAM.NEW.SIGNIN.USING.SAML")%></button>
		<%
		}
		%>	
	 		</div>
			...
			<% 
					...
					for (Integer idpCode : idpCodes) 
					{
						...
						if (provider != null) 
						{
							if (Util.isCommonFSEnabledFor(provider.name())) 
							{
