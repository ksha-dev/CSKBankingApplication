...
//  });
  ...
  var GeoFencing = ZResource.extendClass({
	  resourceName: "GeoFencing",//No I18N
	  attrs : ["location_info","location_type","allowed_clients","name"], // No i18N
	  parent : User
  });
  ...
  var AllowedIPObj = ZResource.extendClass({
	  ...
	  attrs : ["ip_name", "ips", "client_apps"], // No i18N
	  parent : User
  });
...
//  var POLICY="Policy";//No I18N 
  ...
  var GEOLOCATION = "GeoFencing";//No I18N
  ...
  var ThirdPartyAccess="ThirdPartyAccess";//No I18N
  ...
  var profile_data, security_data, sessions_data,history_data,settings_data,groups_data,tfa_data,privacy_data,compliance_data,org_data,thirdpartyaccess_data;
...
$security = 
{
		...
		{
			$(window).unbind("scroll");
			var securityDataUrl = new URI(User,"self","self").include(POLICY).include(PASSWORD).include(AllowedIP).include(AppPasswords).include(Device_logins);	//No I18N
			if(show_geofenching){
				securityDataUrl = securityDataUrl.include(GEOLOCATION);
			}
			if(security_html)
			{
				securityDataUrl.GET().then(function(resp)
				{
			...
			{	
				// Template.create({screenname: 'profile'}).getURI('profile').GET()
				$.when( Template.GET("security"), securityDataUrl.GET())//No I18N
				    .then(function(template,detail) 
				    {
...
function load_profile()
{
  ...
  hide_pref_option = profile_data.is_photo_permission_disabled;
  photoPermission = profile_data.photo_permission;
...
function load_security()
{
	...
	if(show_geofenching){
		if(	security_data.GeoLocations.length > 0 ){			 
			var temp_country_list = [];			//once we share states value with this country remove this case
			security_data.GeoLocations.forEach(function(country){
				temp_country_list.push(country.country.toLowerCase());
			});
			security_data.GeoLocations[0].country = temp_country_list;
		}
		load_geofencing_data(security_data.GeoLocations);
	}
...
function load_privacy(panel_id,kyc_id)
{
	...
	if(privacy_data.Privacy.error || Object.keys(kyc_data).length == 0){
		$("#dpa,#myc").hide();
