...
<%
	...
	if(viewName != null){
		...
		<script>
			...
			if(type != "Box"){
				window.opener.stateData["<%out.print(IAMEncoder.encodeJavaScript(request.getParameter("UNIQUEID")));//NO OUTPUTENCODING%>"]["VIEW_NAME"] = '<%out.print(IAMEncoder.encodeJavaScript(request.getParameter("UNIQUEID")));//NO OUTPUTENCODING%>' + '_PERSVIEW_' + <%out.print(AuthenticationUtil.getAccountID());//NO OUTPUTENCODING%>;<%--NO OUTPUTENCODING --%>
//				window.opener.updateState(window.opener.ROOT_VIEW_ID,'_D_RP','PERSONALIZE=true');
		...
		if(type != null && type.equals("Box")){
			TabPersonalizationUtil.addNewViewToGrid(request.getParameter("VIEWNAME"), AuthenticationUtil.getAccountID(), viewName);
		}else {
			String url = request.getSession().getServletContext().getInitParameter("TabUrl");
                                TabPersonalizationUtil.addNewTab(request.getParameter("VIEWNAME"), AuthenticationUtil.getAccountID(), viewName, url);
